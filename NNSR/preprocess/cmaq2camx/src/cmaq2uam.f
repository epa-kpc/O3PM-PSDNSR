...
c        write(*,*) FDESC3D
c        do i = 1, MXDESC3
c        write(*,*)'KB',i,FDESC3D(i)
c          j = INDEX(FDESC3D(i),'/NCOLS3D/')
c          if (j.gt.0) read(FDESC3D(i)(10:),'(i)') nx
c          j = INDEX(FDESC3D(i),'/NROWS3D/')
c          if (j.gt.0) read(FDESC3D(i)(10:),'(i)') ny
c        enddo
...
cc        if (.not.READ3(INFILE,'IGROUP',1,SDATE3D,STIME3D,kstk)) then ! KB add source appt group ID
cc          MESG = 'Cannot read ROW from ' // TRIM(INFILE)
cc          call M3EXIT(PGNAME,SDATE3D,STIME3D,MESG,1)
cc        endif
...
c
      ...
      write(*,*) 'KB!',ibdate,btime,iedate,etime
...
c
      ...
      if (TSTEP3D.eq.10000.and.typ.eq.'ICON') then
