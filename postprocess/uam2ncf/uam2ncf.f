...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c     UAM2NCF converts CAMx outputs (avrg & depn) to netCDF format.
c     This program assumes LCP grid type and hourly data.
c
c
c     HISTORY:
...
c
      ...
      character(16), parameter :: OUTFILE = 'NCFOUT'
      character(16), parameter :: PGNAME = 'UAM2NCF'
...
c     LCP parameters
c
      write(*,*) 'Enter LCP parameters (1st true lat, 2nd true lat,' //
     &           ' central meridian, center lon, center lat):'
      read(*,'(20x,a)') MESG
...
c
      nzo = nz
      if (LAYFLAG) nzo = 1
      write(*,*) 'Enter sigma levels for NZ=',nzo
      read(*,'(20x,a)') MESG
      write(*,*) MESG
...
c
      ...
      FDESC3D(1) = 'netCDF formatted CAMx ' // TRIM(dtyp) // ' output'
